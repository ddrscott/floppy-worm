...
export default class WormBase {
    constructor(scene, x, y, config = {}) {
        ...
        // Collision groups - negative groups don't collide with each other
        this.segmentGroup = this.matter.world.nextGroup(true);
        this.boneGroup = this.matter.world.nextGroup(true);
    ...
    create(x, y) {
        ...
        //
        //     const boneWidth = radiusA * 2;
        //     const boneLength = (radiusA + radiusB) * 2;
        //     const boneX = (segA.position.x + segB.position.x) / 2;
        //     const boneY = (segA.position.y + segB.position.y) / 2;
        //
        //     const bone = this.matter.add.rectangle(boneX, boneY, boneWidth, boneLength, {
        //         friction: 0.8,
        //         frictionStatic: 0.6,
        //         density: 0.01,
        //         restitution: 0.1,
    ...
    update(delta) {
        ...
        this.bones.forEach((bone, index) => {
            if (index < this.segments.length - 1) {
                ...
                bone.position.x = (segA.position.x + segB.position.x) / 2;
                bone.position.y = (segA.position.y + segB.position.y) / 2;
                
                const angle = Math.atan2(segB.position.y - segA.position.y, segB.position.x - segA.position.x) + Math.PI / 2;
                this.Matter.Body.setAngle(bone, angle);
            }
        });
